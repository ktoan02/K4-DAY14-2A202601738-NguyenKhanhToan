...
class QAPair:
    ...
    Fields:
        ...
        context:            Source context (may be empty string or None).
...
class EvalResult:
    ...
    Fields:
        ...
        failure_type:   None if passed, otherwise failure type string.
        context_precision: Float 0-1 or None.
...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        return max(0.0, min(1.0, precision_sum / rel_count))
...
class LLMJudge:
    ...
    def score_response(
        ...
        """
        prompt = f"Question: {question}\nAnswer: {answer}\nRubric: {rubric}\nPlease score each criterion between 0 and 1 in JSON format."
        try:
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        lowest = [k for k, v in scores.items() if v == min_val]
    ...
    def generate_improvement_log(self, failures: list, suggestions: list[str]) -> str:
        ...
        for idx, f in enumerate(failures, 1):
            ...
            sug = suggestions[idx - 1] if idx - 1 < len(suggestions) else (suggestions[0] if suggestions else "N/A")
            lines.append(f"| {f_id} | {f_type} | {rc} | {sug} | Open |")
        return "\n".join(lines)
    ...
    def generate_improvement_suggestions(
        ...
        if categories.get("irrelevant", 0) > 0 or any(f.relevance < 0.5 for f in failures):
            suggestions.append("Refine system prompt instructions and query rewriting to keep response relevant")

        default_suggestions = [
            "Increase chunk size in RAG pipeline to reduce context fragmentation",
            "Add few-shot examples showing complete answers to improve completeness",
            "Implement hallucination checker to filter unsupported claims",
            "Optimize vector search / BM25 parameters to improve retrieval precision",
        ]
